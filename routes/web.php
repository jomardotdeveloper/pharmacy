...
use App\Http\Controllers\ReorderingController;
...
Route::get("/login",  [LoginController::class, "login"])->name("login.login.2")->middleware("checkauth");
...
Route::resource("canvassings", CanvassingController::class)->middleware(["auth", "notification"]);
Route::resource('reorderings', ReorderingController::class)->middleware(["auth", "notification"]);
...
Route::get("/moves", [MoveController::class, "index"])->middleware(["auth", "notification"])->name("moves.index");